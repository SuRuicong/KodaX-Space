...
const PdfViewer = lazy(() =>
  import('./PdfViewer.js').then((m) => ({ default: m.PdfViewer })),
);
const DocxViewer = lazy(() =>
  import('./DocxViewer.js').then((m) => ({ default: m.DocxViewer })),
);
const XlsxViewer = lazy(() =>
  import('./XlsxViewer.js').then((m) => ({ default: m.XlsxViewer })),
);
...
export function RichPreview({ projectRoot, path, kind }: Props): JSX.Element {
  ...
  if (base64 === null) return <div className="p-3 text-xs text-zinc-500">No content.</div>;

  return (
    <Suspense fallback={<div className="p-3 text-xs text-zinc-500">Loading viewer…</div>}>
