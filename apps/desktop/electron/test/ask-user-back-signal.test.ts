...
test('Space ASK_USER_CUSTOM_INPUT_SIGNAL matches the SDK export', async () => {
  const agent = (await import('@kodax-ai/kodax/agent')) as {
    ASK_USER_CUSTOM_INPUT_SIGNAL?: unknown;
  };
  assert.equal(
    typeof agent.ASK_USER_CUSTOM_INPUT_SIGNAL,
    'string',
    'SDK no longer exports ASK_USER_CUSTOM_INPUT_SIGNAL from @kodax-ai/kodax/agent',
  );
  assert.equal(
    ASK_USER_CUSTOM_INPUT_SIGNAL,
    agent.ASK_USER_CUSTOM_INPUT_SIGNAL,
    'Space ASK_USER_CUSTOM_INPUT_SIGNAL drifted from the SDK value - update the schema constant',
  );
});
