...
async function readHandoffFile(filePath: string): Promise<HandoffFileT> {
  ...
  try {
    ...
    const stale = Date.now() - createdAt > HANDOFF_MAX_AGE_MS;
    return {
      id,
      filePath,
...
async function listHandoffs(): Promise<readonly HandoffFileT[]> {
  ...
  const handoffs = await Promise.all(files.map((file) => readHandoffFile(file)));
  return handoffs.sort((a, b) => (b.createdAt ?? 0) - (a.createdAt ?? 0));
}

async function findHandoff(id: string): Promise<HandoffFileT | null> {
...
export function registerHandoffChannels(): void {
  ...
  registerChannel('handoff.list', async () => ({ handoffs: [...(await listHandoffs())] }));

  registerChannel('handoff.accept', async (input) => {
    const handoff = await findHandoff(input.handoffId);
