...
| ID | Feature | 优先级 | 状态 |
|----|---------|--------|------|
| F060 | Workflow 进程事件管线（main→renderer 地基） | Critical | Done |
| F061 | Workflow 进度面板（phase/agent/step 树，Coder-only） | High | Done |
| F062 | Run 生命周期控制（stop/pause/resume/rename/prune） | High | Done |
| F063 | Workflow 库 + 启动 + capsule preflight | High | Done |
| F064 | AMAW 自然语言自启 + Host policy（极简） | Medium | Done |
| F065 | 子 agent 活动遥测面（correlation 归因） | Medium | Done |
| F066 | Workflow 结果 + artifact 展示（桥进 artifactStore，方案 A） | Medium | Done |
