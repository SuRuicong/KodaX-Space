...
| `composer.imageArtifacts` | supported | Space sends PNG/JPEG/WEBP image artifacts through `KodaXContextOptions.inputArtifacts` and now preserves KodaX 0.7.56 source provenance (`clipboard`, `drag-drop`, `file-picker`, `user-inline`) instead of flattening everything to `user-inline`. |
| `composer.mediaHelpers` | planned    | KodaX 0.7.56 exposes public `@kodax-ai/kodax/media` helpers for clipboard normalization, artifact construction, model input capability checks, and validation. Space still needs main-process IPC and UX wiring before native clipboard fallback, GIF direct-path handling, file artifacts, or video semantics are user-facing. |
