...
> Last Updated: 2026-06-22 (v0.1.22 released: package/lockfile/runtime contract/docs are aligned to `0.1.22`; release scope is trusted internal custom providers, config-provider compatibility, Space-owned per-session follow-up queue, ask_user modal bridge coverage, View menu appearance shortcuts, artifact transcript callouts, CSS spinner frame stability, Diff loading polish, streaming spinner caret cleanup, test-mode Electron userData isolation, and release hygiene. v0.1.23 and v0.1.25 remain patch lanes; v0.1.24 remains F105 customer timebox entitlement MVP).
